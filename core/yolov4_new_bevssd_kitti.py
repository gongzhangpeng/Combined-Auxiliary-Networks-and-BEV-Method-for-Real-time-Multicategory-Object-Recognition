...
anchors = anchors_numpy.reshape([cfg.FEATURE_HEIGHT,cfg.FEATURE_WIDTH,2*cfg.NUM_CLASS,7])
    
def cspdarknet53(input_data):
    input_data = common.convolutional(input_data, (3, 3,  3,  32), activate_type="relu")
    input_data = common.convolutional(input_data, (3, 3, 32,  64), downsample=True, activate_type="relu")

    route = input_data
    route = common.convolutional(route, (1, 1, 64, 64), activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 64, 64), activate_type="relu")
    for i in range(1):
        input_data = common.residual_block(input_data,  64,  32, 64, activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 64, 64), activate_type="relu")

    input_data = tf.concat([input_data, route], axis=-1)
    input_data = common.convolutional(input_data, (1, 1, 128, 64), activate_type="relu")
    input_data = common.convolutional(input_data, (3, 3, 64, 128), activate_type="relu")
    route = input_data
    route = common.convolutional(route, (1, 1, 128, 64), activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 128, 64), activate_type="relu")
    for i in range(2):
        input_data = common.residual_block(input_data, 64,  64, 64, activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 64, 64), activate_type="relu")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = common.convolutional(input_data, (1, 1, 128, 128), activate_type="relu")
    input_data = common.convolutional(input_data, (3, 3, 128, 256), activate_type="relu")
    route = input_data
    route = common.convolutional(route, (1, 1, 256, 128), activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 256, 128), activate_type="relu")
    for i in range(8):
        input_data = common.residual_block(input_data, 128, 128, 128, activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 128, 128), activate_type="relu")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = common.convolutional(input_data, (1, 1, 256, 256), activate_type="relu")
    route_1 = input_data
    input_data = common.convolutional(input_data, (3, 3, 256, 512), downsample=True, activate_type="relu")
    route = input_data
    route = common.convolutional(route, (1, 1, 512, 256), activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 512, 256), activate_type="relu")
    for i in range(8):
        input_data = common.residual_block(input_data, 256, 256, 256, activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 256, 256), activate_type="relu")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = common.convolutional(input_data, (1, 1, 512, 512), activate_type="relu")
    route_2 = input_data
    input_data = common.convolutional(input_data, (3, 3, 512, 1024), downsample=True, activate_type="relu")
    route = input_data
    route = common.convolutional(route, (1, 1, 1024, 512), activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 1024, 512), activate_type="relu")
    for i in range(4):
        input_data = common.residual_block(input_data, 512, 512, 512, activate_type="relu")
    input_data = common.convolutional(input_data, (1, 1, 512, 512), activate_type="relu")
    input_data = tf.concat([input_data, route], axis=-1)

    input_data = common.convolutional(input_data, (1, 1, 1024, 1024), activate_type="relu")
...
def YOLOv4(input_layer):
    ...
    route_1, route_2, conv = cspdarknet53(input_layer) #1/2,1/4,1/8
    ...
    conv_branch = tf.concat([route_1,route_2,conv], axis=-1)

    p_map = common.convolutional(conv_branch, (1, 1, 512, 2*cfg.NUM_CLASS), activate=False, bn=False)
    r_map = common.convolutional(conv_branch, (1, 1, 512, 2*cfg.NUM_CLASS*7), activate=False, bn=False)
    conv_bbox = [p_map,r_map]
...
def compute_loss_and_result(conv_bbox,point_cls,point_reg,lable,train=True):
    targets,pos_equal_one,neg_equal_one,point_cls_gt_placeholder,point_reg_gt_placeholder,point_inbox_mask_gt_placeholder=lable
    p_map, r_map=conv_bbox
    p_pos = tf.sigmoid(p_map)
    r_map = tf.reshape(r_map, [-1,r_map.shape[1],r_map.shape[2],2*cfg.NUM_CLASS,7])
    
    pos_equal_one_sum = tf.reduce_sum(pos_equal_one)+1
    neg_equal_one_sum = tf.reduce_sum(neg_equal_one)+1
    point_internal_sum = tf.reduce_sum(point_inbox_mask_gt_placeholder)+1
            #loss
    ...
    point_reg_loss =  point_inbox_mask_gt_placeholder*tf.reduce_sum(smooth_l1(point_reg,point_reg_gt_placeholder,3),axis=-1)/point_internal_sum
    point_reg_loss = tf.reduce_sum(point_reg_loss)
    conf_pos_loss = (-pos_equal_one * tf.math.log( tf.clip_by_value(p_pos , small_addon_for_BCE,1.0))) / pos_equal_one_sum
    conf_neg_loss = (-neg_equal_one * tf.math.log(tf.clip_by_value(1-p_pos , small_addon_for_BCE,1.0))) / neg_equal_one_sum
    ...
    loss = conf_loss + reg_loss + point_cls_loss + point_reg_loss
    result=[prob_output,delta_output]
    child_loss = [conf_loss,conf_pos_loss,conf_neg_loss,reg_loss,point_cls_loss,point_reg_loss]
    train_summary = tf.summary.merge([
            ...
            tf.summary.scalar('train/point_reg_loss', point_reg_loss)
        ])
    valid_summary = tf.summary.merge([
            ...
            tf.summary.scalar('valid/point_reg_loss', point_reg_loss)
        ])
    if train:
...
def predict(sess,probs,deltas,bbox_pad_output, bbox_output,conf_output,class_output,gtbox3d_with_ID,gtbox3d_with_ID_notcare,raw_lidar,bev_f,img,threshold=cfg.RPN_SCORE_THRESH,evaluation=True,vis=True):
    use_tensor = False
        ...
        if gt_box_num !=0 and pre_box_num !=0:
            ...
            for i in range(pre_box_num):
                ...
                if gt_cls !=4:
                    ...
                    if gt_cls == pred_cls or gt_cls==4:
                        cls_TP[pred_cls] += 1
