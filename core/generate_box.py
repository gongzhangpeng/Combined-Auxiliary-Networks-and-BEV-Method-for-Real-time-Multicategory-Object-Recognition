...
def generate_box_tough(conf_preds, box_preds, class_preds, angle_pred,threshold):
    N,H,W,C = conf_preds.shape
    angle_max_arg = tf.cast(tf.argmax(angle_pred,axis=-1),tf.float32)
    angle = angle_max_arg*(2*np.pi/(cfg.ANGLE_DIVIDE_NUM-1))-np.pi
    conf_argmax = tf.argmax(conf_preds,axis=-1)
    conf_max = tf.reduce_max(conf_preds,axis=-1)
    conf_max_reshape = tf.squeeze(conf_max,axis=0)
    class_argmax = tf.argmax(class_preds,axis=-1)
    ...
    num_index = tf.cast(tf.squeeze(conf_argmax,axis=0),tf.int32)
    class_index = tf.cast(tf.squeeze(class_argmax,axis=0),tf.int32)
    index_anchor = tf.stack([h_index,w_index,class_index,num_index],axis=-1)
    anchors_pick = tf.gather_nd(anchors,index_anchor)
    index_box = tf.stack([h_index,w_index,num_index*6],axis=-1)
    x = tf.gather_nd(box_preds_reshape,index_box)[...,tf.newaxis]
    index_box = tf.stack([h_index,w_index,num_index*6+1],axis=-1)
    y = tf.gather_nd(box_preds_reshape,index_box)[...,tf.newaxis]
    index_box = tf.stack([h_index,w_index,num_index*6+2],axis=-1)
    z = tf.gather_nd(box_preds_reshape,index_box)[...,tf.newaxis]
    index_box = tf.stack([h_index,w_index,num_index*6+3],axis=-1)
    h = tf.gather_nd(box_preds_reshape,index_box)[...,tf.newaxis]
    index_box = tf.stack([h_index,w_index,num_index*6+4],axis=-1)
    w = tf.gather_nd(box_preds_reshape,index_box)[...,tf.newaxis]
    index_box = tf.stack([h_index,w_index,num_index*6+5],axis=-1)
    l = tf.gather_nd(box_preds_reshape,index_box)[...,tf.newaxis]
    r = tf.squeeze(angle,axis=0)[...,tf.newaxis]
    #encode
