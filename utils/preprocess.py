...
def makeBVFeature_addpoint_addvoxel_jit(points,voxel_size,coors_range,grid_size,map_rgb,points_yxz,
    ...
    for i in range(N):
        ...
        d = (1+np.sqrt(coor[1]**2+(coor[0]-HEIGHT_HALF)**2)/10)
        map_rgb[coor[1], coor[0], 0] = np.minimum(np.log(map_rgb[coor[1], coor[0], 3]*d+1)/np.log(64),1)
        if points[i,2] > map_rgb[coor[0], coor[1], 1]:
            map_rgb[coor[1], coor[0], 1] = points[i,2]
...
def get_points_gt(point_keep,gtbox3d_with_ID):
    ...
    point_cls = np.ones([2001,5],dtype=np.float32)*1/(100*4)
    point_reg = np.zeros([2001,2],dtype=np.float32)
    point_inbox_mask = np.zeros([2001],dtype=np.float32)
...
def get_points_gt_jit(point_nozeros,box3d,box_cls,indices,point_num_max,box3d_num,point,point_cls,point_reg,point_inbox_mask):
    anchors_h = np.array([1.53, 3.53, 1.76, 1.73] ,dtype=np.float32)#kitti
    anchors_w = np.array([1.63, 2.54, 0.66, 0.60],dtype=np.float32)
    anchors_l = np.array([3.52,6.93,0.48,1.52],dtype=np.float32)
        ...
        for j in range(box3d_num):
            ...
            if (a>0 and b>0 and c>0 and d>0 )or(a<0 and b<0 and c<0 and d<0 ):
                if nozeros_point_num<1000 and p_z>z and p_z<z+h:
                    ...
                    point_cls[point_index,cls_index]=1-1/100
                    point_reg[point_index,:]=(p_x-x)/anchors_d[cls_index],(p_y-y)/anchors_d[cls_index]
                ...
                if zeros_point_num < 1000:
                    point[point_index,:]=p_y,p_x,p_z
                    point_cls[point_index,4]=1-1/100
                    point_reg[point_index,:]=1,1
